...
export default function Console({ outputs, onClear }: ConsoleProps) {
  ...
  const getMessageStyle = (type: string) => {
    switch (type) {
      ...
      case 'success':
      case 'log':
        if (type === 'log' && outputs.some(o => o.content.includes('Success'))) {
  ...
  const emptyConsole = (
    <div className="h-full flex flex-col items-center justify-center p-8">
      ...
      <p className="mt-2 text-sm text-center" style={{ color: getColor('textSecondary') }}>
        Run code to see output here
      </p>
      <p className="mt-1 text-xs text-center" style={{ color: getColor('textTertiary') }}>
  ...
  return (
    <Card
      ...
      className="h-1/2 min-h-[220px] flex flex-col"
    >
      <div
        ref={consoleRef}
        ...
        {outputs.length === 0 ? (
          ...
          <div className="space-y-1">
            {outputs.map((output, index) => (
              ...
              >
                <span className="text-xs mr-2" style={{ color: getColor('textTertiary') }}>
