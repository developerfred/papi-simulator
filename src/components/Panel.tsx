		...
		<div className="space-y-3">
			...
			<div className="text-xs space-y-2">
				...
				<InfoRow
					label="Explorer"
					value={
						<ExternalLink href={network.explorer} networkColor={networkColor}>
				...
				<InfoRow
					label="Faucet"
					value={
						<ExternalLink href={network.faucet} networkColor={networkColor}>
							Get tokens
						</ExternalLink>
					}
				/>
			...
			>
				...
				<span style={{ color: getColor("textSecondary") }}>
					Connect to {network.endpoint} to interact with the {network.name}{" "}
					testnet
				</span>
			</div>
		</div>
...
export default function InfoPanel({ network }: InfoPanelProps) {
	...
	return (
		<Card className="overflow-hidden">
			<Tabs tabs={tabs} variant="boxed" networkColored />
		</Card>
	);
}
