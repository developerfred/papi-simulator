...
export default function NetworkSelector({
	...
	const { isDarkTheme, setCurrentNetworkId, getColor, getNetworkColor } =
		useTheme();
				...
				{networks.map((network) => {
					...
					const buttonStyle = {
						backgroundColor: isSelected
							...
							: isDarkTheme
								...
								: "rgba(0, 0, 0, 0.02)",
						borderColor: isSelected ? networkColor : getColor("border"),
						boxShadow: isSelected ? `0 0 0 1px ${networkColor}` : "none",
					...
					return (
						...
						>
							...
							<NetworkBadge network={network} size="sm" showName={false} />

							<span
								className="font-medium text-sm mt-2"
								style={{
									color: isSelected ? networkColor : getColor("textPrimary"),
								}}
							>
								{network.name}
							</span>
