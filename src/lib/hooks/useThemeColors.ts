...
export function useThemeColors() {
	...
	return useMemo(() => {
		return {
			...
			// Helper functions
			getButtonColors: (variant: string, isNetworkColored = true) => {
				switch (variant) {
					case "primary":
						return {
							background: isNetworkColored
								? getNetworkColor("primary")
								: getColor("info"),
							text: "#FFFFFF",
							hover: isNetworkColored
								? getNetworkColor("secondary")
								: undefined,
						};
					case "secondary":
						return {
					...
					case "outline":
						return {
							...
							text: isNetworkColored
								? getNetworkColor("primary")
								: getColor("textPrimary"),
							border: isNetworkColored
								? getNetworkColor("primary")
								: getColor("border"),
						};
					case "ghost":
