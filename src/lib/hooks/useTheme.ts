...
export function useTheme() {
  ...
  const applyTheme = (isDark: boolean) => {
    ...
    document.documentElement.style.setProperty('--border-color', isDark ? '#333333' : '#e5e5e5')
    document.documentElement.style.setProperty('--card-bg', isDark ? '#1a1a1a' : '#f5f5f5')
    document.documentElement.style.setProperty('--code-bg', isDark ? '#2d2d2d' : '#f0f0f0')
  }
